...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function handleArray = plotAcidDistCurves_Z(resultObject,varargin)
    FIGURE_TEMP = 3300;
    %% Initialization
    if nargin == 2
    ...
    if exist('runsToCompare','var')
        for ii = 1:nRuns
            ...
            legendNotes{ii} = strcat(target(1:min(8,length(target))),...
                ';',incEngy{ii},...
    ...
    for ii=1:nRuns
        ...
        figure(FIGURE_TEMP);
        h = histogram(-this.acid_xyz_accul(:,3),...
            'BinEdges',binEdges,'Normalization',distOption);
        figure(5011)
        plot(binCenter,h.Values);
        hold on
    end
    title('Z Distribution of acids')
    xlabel('Z depth (nm)')
    ...
    title('Culmulative Z Counts of Acids')
    xlabel('Z depth (nm)')
    ...
    for ii=1:nRuns
        this = resultObject{1,ii};
        figure(FIGURE_TEMP)
        h = histogram(-this.acid_fine_xyz_accul(:,3),...
            'BinEdges',binEdges,'Normalization',distOption);
        figure(5013)
        plot(binCenter,h.Values);
        hold on
    end
    title('Z-Distribution of Activation Events')
    xlabel('Z depth(nm)')
    legend(legendNotes,'Location','northeast')
    ...
    title('Cumulatice Z Distribution of Activation Events')
    xlabel('Z depth(nm)')
