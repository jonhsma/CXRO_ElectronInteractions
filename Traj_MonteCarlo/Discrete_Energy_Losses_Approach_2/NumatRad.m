function [yesno,navail,navail_std,pagidx]=NumatRad(xyz,pag_grid,pagimg,rcnrad)
...
idx2=find((xgrid-xref).^2+(ygrid-yref).^2+(zgrid-zref).^2<=radius_old^2);
tmpmask(idx)=1;
% tmpmask(idx2)=0;
...
pag_removable=pag_removable(tmpmask==1); % do this after the idx=.. line so that pagidx below gest the correct indices.

%%%%%%%% defaults if no PAG is found within vicinity
yesno=0;
navail=sum(pag_removable(:));
navail_std=std(pag_removable(:));
pagidx=idx;
    
%%%% if PAGs found within vicinity, update the return variables
if mean(pag_removable(:))~=0
    yesno=1;
    navail=sum(pag_removable(:));
    navail_std=std(pag_removable(:));
    pagidx=idx;
